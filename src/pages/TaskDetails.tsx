...
export function TaskDetails() {
  ...
  // ── Who is allowed to act on this task? ──────────────────────
  // TaskDetails is reachable directly by URL (/tasks/:taskId), not just via
  // the already-scoped My Tasks list, so it needs its own check: only the
  // task's assignee, or an admin, may approve/reject it. An unassigned task
  // (assignee null — e.g. a group task no one has claimed yet) is left
  // actionable rather than blocked, since claiming happens implicitly here.
  const canAct =
    isAdmin ||
    !task?.assignee ||
    (!!user && task.assignee === user.id);

  // ── Complete / Reject task ──────────────────────────────────
  const handleComplete = async () => {
    if (!canAct) {
      setSubmitError('This task is assigned to someone else — you do not have permission to act on it.');
      return;
    }
    if (!decision) {
  ...
  return (
    <>
      <div className="page-head">
        ...
        <div className="page-actions">
          ...
          <button
            ...
            title={canAct ? undefined : 'Assigned to someone else — you cannot act on this task.'}
          >
      ...
      {!canAct && (
        <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', background: 'var(--neutral-soft, #F3F4F6)', border: '1px solid var(--border-strong, #D1D5DB)', borderRadius: 6, padding: '10px 12px', margin: '0 0 16px' }}>
          <LockIcon style={{ width: 16, height: 16, color: 'var(--text-muted)', flexShrink: 0, marginTop: 2 }} />
          <p className="small" style={{ color: 'var(--text-muted)' }}>
            This task is assigned to <strong>{task?.assignee}</strong>, not you. You can view the details below, but only {isAdmin ? '' : 'the assignee or '}an admin can approve or reject it.
          </p>
        </div>
      )}
