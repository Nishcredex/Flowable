...
const mailRouter = require("./mail/routes/mailRoutes");
const { verifyConnection: verifyMailConnection } = require("./mail/config/transporter");
...
app.listen(PORT, () => {
      verifyMailConnection().then((result) => {
  console.log(result.ok ? `✉️  Mail service verified → ${result.message}` : `✉️  Mail NOT verified: ${result.message}`);
});
